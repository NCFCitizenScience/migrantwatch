...
function appendNewInput() {	 
	...
	var iteration = lastRow - 1;
 	var newElement ="<tr id='tr_" + iteration + "' class='sprow'>" 
	        + "<td>" + iteration + ".</td>"
   		+ "<td><input type='text' name='species[]' id='species_" + iteration + "' value='<? echo $speciesHintText; ?>'></td>"
		+ "<td>"
		...
		+ "</td><td><small><a class='sprow_del' id='sprow_del_" + iteration + "' title='Click here to delete this report'  href='#x'>Delete</a></small>" 
		        + "<input value='0' type='hidden' class='delflag' id='delflag_" + iteration + "'>" 
		+ "</td>"
		+ "</tr>";
	...
	$('.sprow_del').click( function() {
	  ...
	  df_id = sprow_id.replace(/^tr_/, 'delflag_');
...
$("#sighting_submit").click(function() {
	...
	for(var i = 1; i < lastRow; i++) {
 		if ( ( i == 1 ) ||  (  (( $('#species_' + i).val() != '' ) && ( $('#species_' + i).val() != speciesHintText )  ) || ( ( $('#obdate_' + i).val() != '' ) && ( $('#obdate_' + i).val() != dateHintText ) ) ) || ( $('#delflag_' + i).val()  != '1') ) {
...
function update_others(){
	var other_name = $('#other_name').val();
		...
		$("#ulink").attr({
                        href: 'userfriend.php?friend=' + other_name + '&TB_iframe=true&height=400&width=600'
                });
